...
{   sum=0;
    ...
    so=so+sumGivenLevel(root, i);

    else

    se=se+sumGivenLevel(root, i);
}
printf("/n/n/n Dif. is :  %d",(so-se));
...
int sumGivenLevel(struct node* root, int level)
{
  if(root == NULL)
    return;
  if(level == 1)
  ...
  {
    sumGivenLevel(root->left, level-1);
    sumGivenLevel(root->right, level-1);
  }
return sum;
}
